...
(function ($, aycsoft) {
    ...
    $.extend(aycsoft, {
        ...
        layerConfirm: function (_msg, callback) {
            top.aycsoft.language.get(_msg, function (msg) {
        ...
        layerForm: function (op) {
            ...
            var r = top.layer.open({
                ...
                success: function (layero, index) {
                    top['layer_' + dfop.id] = aycsoft.iframe($(layero).find('iframe').attr('id'), top.frames);
                    layero[0].aycsoft_layerid = 'layer_' + dfop.id;
                    //如果底部有按钮添加-确认并关闭窗口勾选按钮
                    if (!!dfop.btn && layero.find('.lr-layer-btn-cb').length == 0) {
                        top.aycsoft.language.get('确认并关闭窗口', function (text) {
        ...
        layerClose: function (name, index) {
            ...
            if (IsClose) {
                ...
                top[layero[0].aycsoft_layerid].location.reload();
            }
