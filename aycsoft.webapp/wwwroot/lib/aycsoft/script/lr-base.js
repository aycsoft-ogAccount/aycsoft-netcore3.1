/*
 * 版 本 Aycsoft-ADMS-Core 轻 创敏捷开发框架(http://www.aycsoft.cn)
 * Copyright (c) 2021-present 广州轻创软件信息科技有限公司
 * 创建人：Aycsoft-前 端 开发组
 * 日 期：2017.03.16
 * 描 述：操作类	
 */
top.aycsoft = (function ($) {
    "use strict";
    var aycsoft = {
        // 是否是调试模式
        isDebug: true,
        ...
        loading: function (isShow, _text) {//加载动画显示与否
            ...
            if (!!_text) {
                ...
                top.aycsoft.language.get("正在拼了命为您加载…", function (text) {
                    top.$("#lr_loading_bar_message").html(text);
                });
            }
            if (isShow) {
        ...
        countFileSize: function (size) {
            if (size < 1024.00)
                return aycsoft.toDecimal(size) + " 字节";
            else if (size >= 1024.00 && size < 1048576)
                return aycsoft.toDecimal(size / 1024.00) + " KB";
            else if (size >= 1048576 && size < 1073741824)
                return aycsoft.toDecimal(size / 1024.00 / 1024.00) + " MB";
            else if (size >= 1073741824)
                return aycsoft.toDecimal(size / 1024.00 / 1024.00 / 1024.00) + " GB";
        },
        // 数组复制
        ...
        checkrow: function (id) {
            ...
            if (id == undefined || id == "" || id == 'null' || id == 'undefined') {
                isOK = false;
                top.aycsoft.language.get('您没有选中任何数据项,请选中后再操作！', function (text) {
                    aycsoft.alert.warning(text);
                });
        ...
        alert: {
            success: function (msg) {
                top.aycsoft.language.get(msg, function (text) {
                    toastr.success(text);
                });
