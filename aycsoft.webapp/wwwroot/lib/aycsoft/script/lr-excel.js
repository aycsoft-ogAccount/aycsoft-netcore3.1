...
(function ($, aycsoft) {
    ...
    $(function () {
        function excelInit() {
            if (!!lrModule) {
                // 导入
                aycsoft.httpAsync('GET', top.$.rootUrl + '/LR_SystemModule/ExcelImport/GetList', { moduleId: lrModule.F_ModuleId }, function (data) {
                ...
                // 导出
                aycsoft.httpAsync('GET', top.$.rootUrl + '/LR_SystemModule/ExcelExport/GetList', { moduleId: lrModule.F_ModuleId }, function (data) {
                    if (data && data.length > 0) {
                        var $layouttool = $('.lr-layout-tool-right');
                        var $btnGroup = $('<div class=" btn-group btn-group-sm"></div>');
                        var hasBtn = false;
