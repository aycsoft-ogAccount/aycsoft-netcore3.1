(function ($, aycsoft) {
    ...
    var imageurl = top.$.rootUrl + '/img/jfgrid/';
    ...
    // 单元格数据格式化
    var clcFormatter = function (value, item, row, rowItem, op, callback) {
        if (item.formatter) {
    ...
    var formatterCell = function (value, item, row, rowItem, op) {
        if (value == undefined && item.edit && (item.edit.type == 'radio' || item.edit.type == 'checkbox')) {
            value = item.edit.dfvalue;
            row[item.name] = value;
        ...
        clcFormatter(value, item, row, rowItem, op, function (text) {
            rowItem.text = $('<div>' + (text || '') + '</div>').text();
            rowItem.$cell.attr('title', rowItem.text);
            if (item.edit) {
                rowItem.$cell.css('padding-left', '4px');
                ...
                switch (item.edit.type) {
                    ...
                    case 'radio':   
                        var setData;
                        if (item.edit.op && item.edit.op.data) {
                            setData = item.edit.op.data;
                        }
                        else {
                            setData = item.edit.data;
                        }
                        
                        if (setData) {
                            $.each(setData, function (_index, _item) {
                                if (_item[item.edit.op.value] == text) {
                            ...
                            if (item.edit.datatype === 'dataItem') {
                                aycsoft.clientdata.getAsync('dataItem', {
                                    ...
                                    callback: function (_data, _op) {
                                        _op.rowItem.text = _data.F_ItemName;
                                    }
                                });
                            }
                            else if (item.edit.datatype === 'dataSource') {
                                aycsoft.clientdata.getAsync('sourceData', {
                                    key: text,
                                    ...
                                    aycsoft.clientdata.getAllAsync('dataItem', {
                                        ...
                                        callback: function (_dataes, _op) {
                                            ...
                                            $.each(_dataes, function (_index, _item) {
                                                if (_vmap[_item.F_ItemValue] == '1') {
                                                    _text.push(_item.F_ItemName);
                                                }
                                            });
                    ...
                    case 'datatime':       // 时间
                        if (item.edit.dateformat == "0") {
                            rowItem.text = aycsoft.formatDate(text, "yyyy-MM-dd");
                            row[item.name] = rowItem.text;
                        }
                        break;
                    case 'layer':          // 弹层
                        // 获取下行号
                        ...
                        rowItem.text = text;
                        rowItem.$cell.attr('title', rowItem.text);
                        rowItem.$cell.html((text || '') + '<i class="fa fa-ellipsis-h" value="' + rownum + '" ></i>');
            ...
            if (item.statistics) {// 如果该单元格数据需要统计就进行计算
                ...
                $('#jfgrid_statistic_' + op.id + ' [name="' + item.name + '"]').text(aycsoft.toDecimal(op.running.statisticData[item.name]));
            }
        });
    };
        ...
        switch (col.data.edit.type) {
                    ...
                    aycsoft.clientdata.getAllAsync('dataItem', {
                        ...
                        callback: function (dataes) {
                            ...
                            $.each(dataes, function (_index, _item) {
                                list.push({ id: _item.F_ItemValue, text: _item.F_ItemName, title: _item.F_ItemName, k: _index });
                            });
                            col.data.edit.op.data = list;
            ...
            case 'radio':          // 单选
                ...
                if (col.data.edit.datatype == 'dataItem') {
                    ...
                    $.each(col.data.edit.data || [], function (id, item) {
                        var $point = $('<label><input name="jfgrid_edit_' + op.id + '_' + col.data.name + '" value="' + item.id + '"' + ' type="radio">' + item.text + '</label>');
            ...
            case 'checkbox':       // 多选
                ...
                if (col.data.edit.datatype == 'dataItem') {
                    ...
                    $.each(col.data.edit.data || [], function (id, item) {
                        var $point = $('<label><input name="jfgrid_edit_' + op.id + '_' + col.data.name + '" value="' + item.id + '"' + ' type="checkbox">' + item.text + '</label>');
                        col.$edit.find('div').append($point);
                    });
                }
                break;
    ...
    var _jfgrid = {
        ...
        layout: function ($self, op) {
            ...
            // 没有数据的时候显示的图片
            $self.find('#jfgrid_body_' + op.id).append('<div class="jfgrid-nodata-img" id="jfgrid_nodata_img_' + op.id + '"  ><img src="' + imageurl + 'nodata.jpg"></div>');
        ...
        head: function ($self, op) {
            ...
            // 判断是否有序号列
            if (op.isShowNum) {
                $border.append('<div class="jfgrid-border-cell jfgrid-border-num" style="width:' + op.numRowWitdh + 'px;" ></div>');
                op.running.leftWidth += op.numRowWitdh;//30;
            }
            // 判断是否允许多选
            if (op.isMultiselect) {
            ...
            $.each(op.running.headData, function (_index, _item) {
                ...
                // 翻译
                aycsoft.language.get((_item.data.label || ""), function (text) {
        ...
        rowRender: function ($self, op, _item, bindex, deep) {
            ...
            if (op.isShowNum) {
                ...
                row['jfnum'].$cell.css({ width: op.numRowWitdh, 'top': row['jfnum'].top, 'left': row['jfnum'].left, 'text-align': 'center', 'height': op.rowHeight, 'line-height': (op.rowHeight - 1) + 'px' });
        ...
        reload: function ($self, op) {
            ...
            if (op.isPage) {
                ...
                op.running.pageparam.sidx = op.sidx || op.mainId;
                ...
                aycsoft.httpAsync('GET', op.url, op.param, function (data) {
        ...
        sortCol: function ($self, $et, op) {
            ...
            /*if (op.rowdatas && op.rowdatas.length == 0) {
                return false;
            }*/

            if (head.last && (head.data.sort === true || head.data.sort === undefined) && op.isPage) {
                var isup = true;
        ...
        clickRow: function ($self, $et, op) {
            ...
            $.each(op.running.headData, function (i, col) {
                if (col.data.name == colname) {
                    if (col.data.edit && col.$edit && col.data.edit.type != 'layer' && col.data.isRead != '1') {
                        isEdit = true;
        ...
        expandSub: function ($self, $et, op) {
            ...
            if (rowItem['jfsubGrid'].value) {
                ...
                var subContentId = aycsoft.newGuid();
            ...
            case 'removeRow':
                if (dfop.isMultiselect) {
                    ...
                    $.each(dfop.running.rowdata, function (_index, _item) {
                        if (_item['jfcheck'].value == 1) {
                            dfop.rowdatas.splice(dfop.rowdatas.indexOf(_item['jfgridRowData']), 1);
                            checklist.push(_item['jfgridRowData']);
                        }
                    });
                    var _$self = $self;
