...
(function ($, aycsoft) {
    ...
    $.lrFormComponents = {
        label: {//文本标题
            ...
            property: function ($component) {
                ...
                $html.find('.lr-component-title').each(function () {
                    var $this = $(this);
                    var text = $this.text();
                    top.aycsoft.language.get(text, function (_text) {
                        $this.text(_text);
                    });
                });
                // 标题设置
        ...
        datetime: {//日期框
            ...
            renderTable: function (compont, $row) {//使用表单的时候渲染成table
                ...
                datedefault = aycsoft.formatDate(datedefault, dateformat.replace(/H/g, 'h'));
                var $compont = $('<input value="' + datedefault + '" id="' + compont.id + '" dateformat="' + compont.dateformat + '"   onClick="WdatePicker({dateFmt:\'' + dateformat + '\',qsEnabled:false,isShowClear:false,isShowOK:false,isShowToday:false,onpicked:function(){$(\'#' + compont.id + '\').trigger(\'change\');}})"  type="text" class="form-control lr-input-wdatepicker" />');
                $row.append($compont);
                return $compont;
            }
        },
        ...
        encode: {//单据编码
            ...
            property: function ($component) {
                ...
                if (!!ruleCode) {
                    ...
                    aycsoft.httpAsync('GET', top.$.rootUrl + '/LR_SystemModule/CodeRule/GetList', {}, function (data) {
                        ruleCode = data;
                        $('#lr_component_rulecode').lrselectRefresh({
                            data: ruleCode
                        });
            ...
            renderTable: function (compont, $row) {//使用表单的时候渲染成table
                ...
                aycsoft.httpAsync('GET', top.$.rootUrl + '/LR_SystemModule/CodeRule/GetEnCode', { code: compont.rulecode }, function (data) {
                    if (!$compont.val()) {
                        $compont.val(data);
                    }
                });
        ...
        currentInfo: {//固定信息
            ...
            renderTable: function (compont, $row) {
                ...
                if (compont._userId != undefined && compont._userId != '' && loginInfo.F_UserId != compont._userId) {
                    aycsoft.clientdata.getAsync('user', {
                        key: compont._userId,
                        callback: function (_data) {
                            switch (compont.dataType) {
                                case 'company':
                                    $compont[0].lrvalue = _data.F_CompanyId;
                                    aycsoft.clientdata.getAsync('company', {
                                        key: _data.F_CompanyId,
                                        callback: function (_data) {
                                    ...
                                    break;
                                case 'department':
                                    $compont[0].lrvalue = _data.F_DepartmentId;
                                    aycsoft.clientdata.getAsync('department', {
                                        key: _data.F_DepartmentId,
                                        callback: function (_data) {
                                            if ($compont.attr('data-flag') != "1") {
                                                $compont.val(_data.F_FullName);
                                            }
                                    ...
                                    break;
                                case 'user':
                                    $compont.val(_data.F_RealName);
                                    $compont[0].lrvalue = compont._userId;
                                    break;
                                case 'time':
                                    $compont[0].lrvalue = aycsoft.formatDate(new Date(), 'yyyy-MM-dd hh:mm:ss');
                    ...
                    switch (compont.dataType) {
                        case 'company':
                            $compont[0].lrvalue = loginInfo.F_CompanyId;
                            aycsoft.clientdata.getAsync('company', {
                            ...
                            break;
                        case 'department':
                            $compont[0].lrvalue = loginInfo.F_DepartmentId;
                            aycsoft.clientdata.getAsync('department', {
                            ...
                            break;
                        case 'user':
                            $compont.val(loginInfo.F_RealName);
                            $compont[0].lrvalue = loginInfo.F_UserId;
                            break;
                        case 'time':
                            $compont[0].lrvalue = aycsoft.formatDate(new Date(), 'yyyy-MM-dd hh:mm:ss');
                            $compont.val($compont[0].lrvalue);
        ...
        guid: {//固定信息
            ...
            renderTable: function (compont, $row) {
                ...
                $compont[0].lrvalue = aycsoft.newGuid();
                $compont.val($compont[0].lrvalue);
                $row.hide();
                $row.append($compont);
                return $compont;
        ...
        girdtable: {
            ...
            renderTable: function (compont, $row) {
                ...
                if (!!compont.preloadDb && !!compont.preloadTable) {
                    ...
                    aycsoft.httpAsync('GET', top.$.rootUrl + '/LR_SystemModule/DatabaseTable/GetTableDataAllList', { dbCode: compont.preloadDb, tableName: compont.preloadTable }, function (data) {
            ...
            select: function (item) {
                if (!!item) {
                    ...
                    if (!!fieldMap[designerDfop.dbId + item.name]) {
                        ...
                        aycsoft.httpAsync('GET', top.$.rootUrl + '/LR_SystemModule/DatabaseTable/GetFieldList', { dbCode: designerDfop.dbId, tableName: item.name }, function (data) {
                    ...
                    aycsoft.clientdata.getAllAsync('dataItem', {
                        ...
                        callback: function (dataes) {
                            ...
                            $.each(dataes, function (_index, _item) {
                                if (_item.F_ParentId == "0") {
                                    list.push({ id: _item.F_ItemValue, text: _item.F_ItemName, k: _index });
                                }
                            });
        ...
        $('#lr_component_dataSourceId').lrselect({
            ...
            select: function (item) {
                if (item) {
                    ...
                    //获取字段数据
                    aycsoft.httpAsync('GET', top.$.rootUrl + '/LR_SystemModule/DataSource/GetDataColName', { code: item.F_Code }, function (data) {
                        var fieldData = [];
                        for (var i = 0, l = data.length; i < l; i++) {
                            var id = data[i];
    ...
    function toGirdheadData(data, hideData, $compont) {
        ...
        $.each(data, function (_index, item) {
            var isAdd = true;
            var point = { id: item.id, label: item.name, name: item.field || aycsoft.newGuid(), width: parseInt(item.width || 100), align: item.align || "left" };
            if (item._isEdit == 1) {
                point.isRead = '1';
            }
            switch (item.type) {
                ...
                case 'layer':          // 弹层
                    ...
                    if (item.dataSource == "0") {
                        ...
                        point.edit.op.param = { code: item.dataSourceId };
                    };
