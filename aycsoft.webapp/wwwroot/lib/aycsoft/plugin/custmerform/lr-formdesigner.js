...
(function ($, aycsoft) {
    ...
    $.lrCustmerFormDesigner = {
        init: function ($self, op) {
            ...
            _html += '</div>';
            _html += '<div class="lr-custmerform-designer-layout-center-bg"><img src="' + top.$.rootUrl + '/img/tableform.png" /></div>';
            ...
            $('#lr_custmerform_designer_layout_area_' + dfop.id + ' .lr-scroll-box').sortable({
                ...
                stop: function (event, ui) {
                    ...
                    if (!!componttype) {//如果是第一次移入，需要对单元项进行初始化处理
                        ...
                        $compont[0].dfop.id = aycsoft.newGuid();
                    ...
                    if (dfop.data[i].componts[j].type != 'label' && dfop.data[i].componts[j].type != 'html') {
                        ...
                        if (table != '' && field != '') {
                            if (!!_data[table + '_' + field]) {
                                aycsoft.alert.error('【' + title + '】绑定数据表字段与【' + _data[table + '_' + field] + '】重复！');
                                res = false;
                            }
                            else {
                            ...
                            if (dfop.data[i].componts[j].type == 'gridtable' || dfop.data[i].componts[j].type == 'girdtable') {
                                if (table == '') {
                                    aycsoft.alert.error('【表格项】请绑定数据表！');
                                    res = false;
                                }
                            }
                            else {
                                aycsoft.alert.error('【' + title + '】请绑定数据表！');
                                res = false;
                            }
                        }
                    }
