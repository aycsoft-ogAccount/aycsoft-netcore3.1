        ...
        clickWorkArea: function (e) {
            ...
            if (!dfop.isPreview) {
                ...
                if (type == 'endround') {
                    name = "结束";
                    if (dfop.hasEndround) {
                        aycsoft.alert.error('只能有一个结束节点');
                        return false;
                    }
                }
                $.lrworkflow.addNode($this, dfop, { id: aycsoft.newGuid(), name: name, left: X, top: Y, type: type });
            }
        },
        // 取消所有结点/连线被选定的状态
        ...
        initNodeEvent: function ($workArea) {
            ...
            if (!dfop.isPreview) {
                ...
                $workArea.delegate(".lr-workflow-nodespot", "mouseup", { $workArea: $workArea }, function (e) {
                    ...
                    if (lineStart) $.lrworkflow.addLine(dfop, { id: aycsoft.newGuid(), from: lineStart.id, to: nodeData.id, sp: lineStart.position, ep: position, name: "" });
                });
                //绑定结点的删除功能
