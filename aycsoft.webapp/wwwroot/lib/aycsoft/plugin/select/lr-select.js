 ...
 * 创建人：轻 创-前端开发组
 * 日 期：2017.03.22
 * 描 述：aycsoftSelect（普通，多选，树形数据，gird，搜索，输入框选择器）-渲染数据在点击的时候触发，考虑到在一个表单上有超级多的下拉框的绑定情况（这里需要考虑赋值的特殊性）
 */
(function ($, aycsoft) {
    "use strict";
    ...
    $.lrselect = {
        ...
        initRender: function (dfop, $self) {
            $('#aycsoft_select_option_' + dfop.id).remove();
            var $option = $('<div class="lr-select-option" id="aycsoft_select_option_' + dfop.id + '"></div>');

            var $optionContent = $('<div class="lr-select-option-content"></div>');
            var $ul = $('<ul id="aycsoft_select_option_content' + dfop.id + '"></ul>');
        ...
        defaultRender: function (dfop) {
            var $ul = $('#aycsoft_select_option_content' + dfop.id);
            $ul.html("");
            if (dfop.placeholder) {
            ...
            for (var i = 0, l = dfop.data.length; i < l; i++) {
                ...
                if (!item._lrhide) {
                    if (!!dfop.multipleMapValue && dfop.multipleMapValue[i] != undefined) {
                        var $li = $('<li data-value="' + i + '" class="lr-selectitem-li" ><img class="lr-select-node-cb" src="' + top.$.rootUrl + '/img/aycsofttree/checkbox_1.png">' + item[dfop.text] + '</li>');
                        $ul.append($li);
                    }
                    else {
                        var $li = $('<li data-value="' + i + '" class="lr-selectitem-li" ><img class="lr-select-node-cb" src="' + top.$.rootUrl + '/img/aycsofttree/checkbox_0.png">' + item[dfop.text] + '</li>');
                        $ul.append($li);
                    }
                }
            }
        ...
        treeRender: function (dfop) {
            var $option = $('#aycsoft_select_option_' + dfop.id);
            ...
            if (dfop.type != 'multiple') {
                ...
                if ($et.hasClass('lr-selectitem-li') || $et.hasClass('lr-select-node-cb')) {
                    ...
                    if (_index == -1) {
                        ...
                        $option.find('.lr-select-node-cb[src$="checkbox_1.png"]').attr('src', top.$.rootUrl + '/img/aycsofttree/checkbox_0.png');
                        $option.fadeOut(150);
                        $this.removeClass('lr-select-focus');
                    }
                    else {
                        ...
                        if (dfop.multipleMapValue[_index] == undefined) {
                            ...
                            $cbobj.attr('src', top.$.rootUrl + '/img/aycsofttree/checkbox_1.png');
                        }
                        else {
                            dfop.multipleValue = [];
                            dfop.multipleText = [];
                            ...
                            selected = false;
                            $cbobj.attr('src', top.$.rootUrl + '/img/aycsofttree/checkbox_0.png');
                        }
        ...
        treeNodeClick2: function (item, $item) {
            ...
            dfop.currtentItems = [];
            if (item.value == '-1') {
                $item.parents('.lr-select-option').fadeOut(150);
        ...
        defaultValue: function ($self, type) {
            ...
            if (dfop.type == 'multiple') {
                ...
                dfop.multipleValue = [];
                dfop.multipleText = [];
                $('#aycsoft_select_option_' + dfop.id).find('.lr-select-node-cb').attr('src', top.$.rootUrl + '/Content/images/aycsofttree/checkbox_0.png');
            }
    ...
    $.fn.lrselect = function (op) {
        ...
        if (dfop.url) {
            if (dfop.isAsync) {
                aycsoft.httpAsync(dfop.method, dfop.url, dfop.param, function (data) {
                    $self[0]._lrselect.dfop.data = data || [];
                    $self[0]._lrselect.dfop.backdata = data || [];
                    dfop.isload = true;
                });
            } else {
                aycsoft.httpSync(dfop.method, dfop.url, dfop.param, function (data) {
                    $self[0]._lrselect.dfop.data = data || [];
                    $self[0]._lrselect.dfop.backdata = data || [];
                    dfop.isload = true;
                });
        ...
        function _fn(dfop) {
            if (dfop.isload == false) {
                ...
                switch (dfop.type) {
                    ...
                    case 'treemultiple':
                        $.lrselect.render(dfop);
                        $('#aycsoft_select_option_' + dfop.id + " .lr-select-option-content").lrtreeSet('setCheck', value.split(','));
                        return false;
                    case 'gird':
                        break;
                    case 'multiple':
                        ...
                        dfop.multipleText = [];
                        if (dfop.isrender) {
                            $('#aycsoft_select_option_' + dfop.id).find('.lr-select-node-cb[src$="checkbox_1.png"]').attr('src', top.$.rootUrl + '/Content/images/aycsofttree/checkbox_0.png');
                        }
                        ...
                        for (var i = 0, l = dfop.data.length; i < l; i++) {
                            ...
                            if (_arrayIndex != -1) {
                                ...
                                if (dfop.isrender) {
                                    $('#aycsoft_select_option_' + dfop.id).find('[data-value="' + i + '"] .lr-select-node-cb').attr('src', top.$.rootUrl + '/Content/images/aycsofttree/checkbox_1.png');
                                }
                                if (dfop.select) {
