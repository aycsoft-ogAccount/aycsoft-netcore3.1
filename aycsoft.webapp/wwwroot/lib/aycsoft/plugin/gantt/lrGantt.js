...
(function ($, aycsoft) {
    ...
    Date.prototype.MaxDayOfDate = function () {
        var myDate = this;
        var date1 = aycsoft.parseDate(aycsoft.formatDate(myDate, 'yyyy-MM-01 00:00:00'));
        ...
        loadData: function ($self, op) {
            ...
            if (op.isPage) {
                ...
                if (op.url) {
                    ...
                    aycsoft.httpAsync('GET', op.url, _param, function (data) {
                        aycsoft.loading(false);
                        op.data = data || [];
                        _gantt.renderData($self, op);
                        op.onRenderComplete && op.onRenderComplete(op.data);
                    ...
                    if ($this.hasClass('lr-gantt-tree-node-expanded')) {
                        ...
                        if (!node.complete) {
                            if (!node._loading) {
                                ...
                                aycsoft.httpAsync('GET', url, param, function (data) {
                                    if (data) {
                                        node.children = data;
    ...
    var DateUtils = {
        ...
        parseDate: function (day, strInterval) {
            switch (strInterval) {
                case 'd':
                    return aycsoft.parseDate(aycsoft.formatDate(day, 'yyyy-MM-dd 00:00:00'));
                    break;
                case 'w':// 获取当前周的第一天
                    var d = aycsoft.parseDate(aycsoft.formatDate(day, 'yyyy-MM-dd 00:00:00'));
                    var w = d.getDay();
                    return d.DateAdd('d', (1 - w));
                    break;
                case 'm':
                    return aycsoft.parseDate(aycsoft.formatDate(day, 'yyyy-MM-01 00:00:00'));
                    break;
                case 'h':
                    return aycsoft.parseDate(aycsoft.formatDate(day, 'yyyy-MM-dd hh:00:00'));
                    break;
                default:
                    return aycsoft.parseDate(aycsoft.formatDate(day, 'yyyy-MM-dd 00:00:00'));
                    break;
    ...
    $.fn.lrGantt = function (op) {
        ...
        var dfop = {
            ...
            isPage: false,
            rows: 50,
            imgUrl: top.$.rootUrl + '/img/jfgrid/nodata.jpg',
            cbiconpath: top.$.rootUrl + '/img/aycsofttree/',
