    ...
    var methods = {
        ...
        bindEvent: function ($this, $scroll, $vertical, $horizontal) { // 绑定监听事件
            ...
            $this.mousewheel(function (event, delta, deltaX, deltaY) {
                ...
                if ($select.length > 0) {
                    var selectId = "aycsoft_select_option_" + $select.attr('id');
                    $('#' + selectId).slideUp(150);
                    $select.removeClass('lr-select-focus');
                }
                var op = $this[0].op;
                var d = 0;// var d = delta * 20;
                if (op.sh > op.h) {
                    d = delta * event.deltaFactor;//d = (op.sh - op.h) * delta / 100 ;
                    //console.log(op.sh - op.h);
                    op.oldsy = op.sy;
                    op.sy = op.sy - d;
                    setTimeout(function () {
                    ...
                    d = delta * event.deltaFactor;//d = (op.sw - op.w) * delta / 100 ;
                    op.oldsx = op.sx;
                    op.sx = op.sx - d;
                    setTimeout(function () {
...
})(window.jQuery, top.aycsoft, window);