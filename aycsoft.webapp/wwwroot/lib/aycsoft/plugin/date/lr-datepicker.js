...
(function ($, aycsoft) {
    $.lrdate = {
        init: function ($self) {
            ...
            $self.attr('type','lr-search-date-type');
            var $text = $('<div class="lr-search-date-text" id="lr_search_date_' + dfop.id + '" >请选择日期查询</div>');
            ...
            // 更新为laydate
            laydate.render({
                elem: '#lr_search_date_datepicker1_' + dfop.id
                , position: 'static'
                , showBottom: false
                , theme: '#2F94EE'
            ...
            laydate.render({
                elem: '#lr_search_date_datepicker2_' + dfop.id
                , position: 'static'
                , showBottom: false
                , theme: '#2F94EE'
                , min: dfop.minDate || '1900-1-1'
            ...
            $customDate.find('a').on('click', function (e) {
                ...
                $('#lr_search_date_container_' + dfop.id).addClass(dfop.type || 'left');
            ...
            $clearDate.find('a').on('click', function (e) {
                ...
                $self.find('.lr-search-date-content-btns a.active').removeClass('active');
                $text.html("");
                dfop._begindate = "1753-01-01"
                dfop._enddate = "3000-01-01"
                dfop.selectfn && dfop.selectfn("1753-01-01", "3000-01-01");
            });
            ...
            $datepickerBtn.find('a').on('click', function () {
                ...
                var $text = $self.find('#lr_search_date_' + dfop.id);
                $container.hide();
                var label = aycsoft.formatDate(dfop._begindate, 'yyyy-MM-dd') + '~' + aycsoft.formatDate(dfop._enddate, 'yyyy-MM-dd');
                $text.html(label);
                dfop.selectfn && dfop.selectfn(dfop._begindate, dfop._enddate);
            });
        },
        ...
        select: function ($self, value) {
            ...
            switch (value) {
                case 'preM':
                    var d = aycsoft.getPreMonth();
                    dfop._begindate = d.begin;
                    dfop._enddate = d.end;
                    break;
                case 'currentM':
                    var d = aycsoft.getMonth();
                    dfop._begindate = d.begin;
                    dfop._enddate = d.end;
                    break;
                case 'preJ':
                    var d = aycsoft.getPreQuarter();
                    dfop._begindate = d.begin;
                    dfop._enddate = d.end;
                    break;
                case 'currentJ':
                    var d = aycsoft.getCurrentQuarter();
                    dfop._begindate = d.begin;
                    dfop._enddate = d.end;
                    break;
                case 'yS':
                    var d = aycsoft.getFirstHalfYear();
                    dfop._begindate = d.begin;
                    dfop._enddate = d.end;
                    break;
                case 'yX':
                    var d = aycsoft.getSecondHalfYear();
                    dfop._begindate = d.begin;
                    dfop._enddate = d.end;
                    break;
                case 'preY':
                    var d = aycsoft.getPreYear();
                    dfop._begindate = d.begin;
                    dfop._enddate = d.end;
                    break;
                case 'currentY':
                    var d = aycsoft.getYear();
                    dfop._begindate = d.begin;
                    dfop._enddate = d.end;
                    break;
                default:
            ...
            $('#lr_search_date_container_' + dfop.id).removeClass(dfop.type || 'left');
